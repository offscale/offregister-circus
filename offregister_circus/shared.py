...
def setup_circus(circus_virtual_env, env, home, log_dir, name, port,
                 remote_user, app_virtual_env, working_dir, wsgi_file,
                 uname, cmd_args=None, cmd='gunicorn', shell='/bin/bash'):
    conf_dir = '/etc/circus/conf.d'
    remote_circus_ini = '{conf_dir}/circus.ini'.format(conf_dir=conf_dir)

    assert cmd_args is not None or wsgi_file is not None, 'cmd_args or wsgi_file must be defined'

    circus_ini_context = {
        'CMD': cmd,
        'CMD_ARGS': ('-w 3 -t 60 -b 127.0.0.1:{port:d} {wsgi_file}'.format(port=port,
                                                                           wsgi_file=wsgi_file)
                     if cmd_args is None else cmd_args),
    ...
    circus_ini_content = circus_ini_content.format(**circus_ini_context)  # Code is here because I like to error early
    ...
    if not exists(remote_circus_ini):
        ...
        if uname.startswith('Darwin'):
            ...
            upload_template(
                configs_dir('circusd.service'), '/etc/systemd/system/',
                context=circusd_context, use_sudo=True
            )
        else:
            upload_template(
                configs_dir('circusd.conf'), '/etc/init/',
                context=circusd_context,
                use_sudo=True)
